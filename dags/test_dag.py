...
with DAG('tests_dag', description='Python DAG', schedule_interval=timedelta(1), start_date=datetime(2021, 4, 17), template_searchpath=(os.environ["AIRFLOW_HOME"]+'/dag/repo/postgres_files'), catchup=False) as dag:
	...
	t3=BashOperator(task_id='check_new_file', bash_command='$AIRFLOW_HOME/dags/repo/store_files/clean_store_transactions.csv', retries=2, retry_delay=timedelta(seconds=15))
	t4 = PostgresOperator(task_id='create_postgres_table',postgres_conn_id="postgres_conn", sql="create_table.sql")
